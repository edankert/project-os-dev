...
## Definition of Done
- [ ] `independent-review/SKILL.md:47-50` asks the reviewer for every finding, each labelled reproduced or not reproduced.
- [ ] The same file says the repro filter applies when findings are transcribed into `ISS-*` notes, which is a separate pass by construction.
- [ ] This repo's `tools/scripts/review-external.py` drops "a finding without a repro is not a finding" from the reviewer's output schema and applies it at transcription instead (docstring point 4, and the schema it describes).

## Steps
- [ ] Change the skill text first; the runner's docstring cites it.
- [ ] Check whether any recorded review verdict depended on the old filter before changing the schema.
