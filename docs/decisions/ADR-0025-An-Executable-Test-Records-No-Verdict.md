...
alternatives: ["Option 1, the runner stamps the note (ADR-0010 kept)"]
consequences: ["run-tests.py never writes; in CI an unrunnable test fails the run", "the validator treats a command: test as settled by CI and warns on a stamped verdict until 2026-12-02", "manual tests and acceptance checks are unchanged", "a repo whose tests run a sibling checkout must clone it in CI"]
supersedes: ""
superseded: ""
...
- The validator treats a test with a `command:` as settled by CI in the verification gate, and warns, with a dated promotion under ADR-0011, on a `command:` test that still carries a verdict field. Measured by the independent review at landing, with the check over the fleet's trees: 98 notes (your-trainer 69, most carrying only an `exit_code:`; your-health 21; your-sudoku 4; this repo 4 before it stripped them; project-os-cockpit 0); by status 29 and 2 at `ready`. The count first written here, 33 and 5, was wrong.
...
- Which sessions gate on the tests is now a property of CI, not of the note. A repo whose CI does not run `run-tests.py` has no gate for its executable tests until it does; the template's CI seed runs it. In CI an unrunnable command fails the run, so a repo whose tests run a sibling checkout must clone it there; this repo's workflow does. Pre-commit does not run the tests, so a done task can sit over a failing executable test until CI runs.
