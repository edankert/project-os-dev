...
## Definition of Done
- [ ] `SNAPSHOT.md` says a `title` is at most twelve words.
- [ ] It says `goal:` and `note:` are at most two sentences each, and that anything longer belongs in the note body.
- [ ] It names the destination: the feature or task note's own sections, which is where a reader looking for the detail already goes.
- [ ] `change.md`, `issue.md` (Problem) and `feature.md` (Goal) each ask for a summary of two or three sentences, point first.

## Steps
- [ ] Write the numbers into `SNAPSHOT.md:63,68-69`.
- [ ] Add the one-line instruction to the three templates.
