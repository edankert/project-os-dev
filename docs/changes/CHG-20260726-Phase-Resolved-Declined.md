...
Authored by model:claude-opus-5 (commit trailer on 5c4319d), reviewed by model:claude-fable-5 — same model family; this remains harm reduction, not the cross-vendor independence QUALITY.md requires, and a different-family or human pass is still owed.

**Verified:** every [[ISS-0014]] Next Action is done on the merits — the duplicated tail is gone (1619 lines, one entry guard, zero duplicated top-level definitions by independent AST scan; the full 12a7c70→5c4319d diff reviewed end-to-end contains only the intended changes); all 12 fleet files byte-identical including `project-os-bench` and the cockpit bundle; all 11 repos validate 0 errors, `--self-check` 0, `sync-snapshot --check` 0, cockpit suite 315 passed; the dict/underscore/nested/lowercase evasions are all closed and nine further mutation branches fire with the claimed messages; `REVIEW_SETTLED_STATUSES` is hoisted, registered and drift-checked, and the replacement claim ("no multi-value inline status collection remains") is exact by independent AST scan; metrics parity confirmed by a different method (12a7c70 validator vs current recorded metrics: zero diffs, 18 keys, 11 repos); the `decision` alias is real (one decision-typed note fleet-wide, your-health ADR-0006 `accepted`; vocabulary matches STATUSES.md `[[adr]]`; deleting the key turns `--self-check` red); the qualified prefix-map row is now accurate in both directions.

**Why changes-requested a fourth time — filed as [[ISS-0015]], the narrowest round yet:** (1) the walker's cap at nesting depth 4 refutes "to any nesting depth" in the docstring and [[TST-0002]] (a depth-5 status collection evades, demonstrated; ISS-0014's own Resolution correctly says "depth 4", so the file disagrees with itself); (2) "a decision-typed note's status was validated against nothing" is imprecise — the pre-fix gate was a hash-seed coin flip (caught on 5/10 seeds, demonstrated against 12a7c70), because STATUS-VALUE picks its type via `next(iter(expected_types))`, and that pick is still there: deterministic today only because the `adr` and `decision` vocabularies coincide, re-splittable by any downstream STATUSES.md that customises `[[adr]]`. Also noted: this note's "Files changed (11 across 10 repos)" table predates `project-os-bench` (the fleet is 12 files across 11 repos). The substance of round three's fixes is accepted in full; the requested changes are two exact sentences and one deterministic type pick.
...
## Follow-up: ISS-0015 (round five)

Two findings, both held, both small. The completeness walk capped at nesting depth 4 while the docstring one paragraph above promised "any nesting depth" — so the cap is gone rather than the sentence trimmed, with cycle-safety by identity doing the job the cap was standing in for. And `STATUS-VALUE` picked its type with `next(iter(expected_types))` over an unordered set, making a `decisions`-collection status check a per-run coin flip: measured at 6 of 12 `PYTHONHASHSEED` values before the fix, 12 of 12 after it takes the union.

The second finding also corrected ISS-0014's own words. "Validated against nothing" describes a dead gate; the truth was a coin flip, which is worse, because it passes CI and fails locally and reads as flakiness rather than a missing check. Corrected wherever it appeared, including the code comment.

Five rounds. The defect count is converging — four findings in round two, two in round five, nothing structural since round three — but the shape has not changed once: a coverage claim written slightly wider than the code, agreed with by every mechanical check, caught only by a reviewer attacking the guard. Five for five.
