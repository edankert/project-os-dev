...
- **A failing test is fixed, whoever broke it and whenever** (Edwin, 2026-09-19). A test that fails in any run you make is fixed before the work closes, even when the work did not touch its code and it failed before the work started. "Pre-existing" and "unrelated" are not reasons to leave it red. A test that fails because of the machine rather than the code, such as a missing build or a device, is made to skip with that reason, or the step it needs is added to the run. The only thing that waits is a fix that needs the owner's decision: ask in the same turn, with a recommendation. Reason: a suite that is allowed to stay red stops being read, and the next real failure hides among the known ones.
- **The filing bar.** A finding becomes an `ISS-*` only when its fix needs a decision the owner has not made, the defect is in code the feature did not change (a failing test is never this case: see the rule above), or the fix is too large for the session. Anything else is fixed, or recorded in the reviewed note's review section and dropped. A stale figure, a wording nit or a duplicated history line is not an issue.
