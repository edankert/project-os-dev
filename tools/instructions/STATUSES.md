...
| `test` | `passing` / `failing`, or `retired` | — | the author, and only for a manual test, which carries `last_verified:` and goes stale. A test carrying a `command:` holds no verdict (project-os-dev ADR-0025, after cockpit ADR-0038) |
...
Which snapshot fields are derived from the notes, and by what, is `LIFECYCLE.md`, "Mandatory Automated Documentation" (ADR-0009).
...
- **Staleness** is computed from `last_verified:` by the validator; a typed `stale` would reintroduce the assertion problem ADR-0010 removed. A manual verification goes stale after **90 days**, which a repo overrides with `verification.staleness_days` in `SNAPSHOT.yaml`.
...
- Plans carry no `id:`; a `PLAN-FEAT-0012` id would resolve to `FEAT-0012` in the ID parser. They are found by `type: [[plan]]` (validator PLAN-ID; PLAN-STATE checks their status).

## `[[reference]]`
- Allowed: `active`, `deprecated`

## `[[surface]]`
- Allowed: `active`, `retired`, `superseded`
- A surface is not *done*: it exists until the product stops having it. `retired` says the place is gone; `superseded` says another surface took it over, named in `superseded_by:`.
