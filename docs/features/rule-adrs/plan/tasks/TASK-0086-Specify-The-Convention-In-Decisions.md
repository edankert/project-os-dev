...
- [x] No other file added by this feature restates any of it — evidence: read against the TASK-0087/TASK-0088 diffs in `6ca15f4`; the template comment carries the three headings and a pointer, SCHEMAS.md carries one pointing sentence, both skills link to the section by name and add only the behaviour that is theirs. **This box was ticked wrongly at first landing**: independent review (`244baec`) found two sentences restated near-verbatim in the skills — "the rule is not ready to be decided" (adr-authoring) and "one instance is a bug, two is a domain" (issue-intake) — exactly the defect this task's own "Why this file" section names. Both became pointers at project-os `4aa2238`; the box is ticked against that state, with the miss recorded rather than erased.
