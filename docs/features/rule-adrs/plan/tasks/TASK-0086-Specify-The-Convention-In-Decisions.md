...
- [ ] `tools/instructions/DECISIONS.md` in `~/Dev/repos/project-os` carries the specification: three sections with semantics, the second-issue harvest trigger, the from-principle exception, the landing pattern, and the `## Options` answer.
- [ ] The section links to [[ADR-0023]] for the decision and [[ADR-0022]] for why it is a convention rather than a kind.
- [ ] No other file added by this feature restates any of it — verified by reading the diffs of TASK-0087 and TASK-0088 against this section.
- [ ] `bash tools/scripts/validate-docs.sh` clean in `project-os` after the edit.
