...
Two mechanisms were in scope for that question and only one of them is expensive, so this ADR separates them. The **independent review gate** (`QUALITY.md`) fires on three status transitions and had produced 18 recorded verdicts when this was written: 16 approved, 2 changes-requested. That is not where the time goes, and a changes-requested rate around 10% is real signal rather than a rubber stamp, so this ADR proposes no change to it. The gate then proved the point on this ADR's own implementation: the review of REQ-0027 returned `changes-requested` and caught, among other things, that the first justification for the amendment below was invalid and that `BASE-STATUS` had been described in a commit that did not contain it. The **docs-audit quiescence rule** is the expensive one, and it is what follows.
...
3. **Run passes in parallel, not in sequence.** Three independent sweeps at one commit, their findings combined (union, or kept on two-of-three agreement — settled in the Decision). One wall-clock interval instead of three, and it is the configuration the research scored highest. Costs three times the tokens of a single pass.
...
5. **Adopt 2, 3 and 4 together, and move the human checkpoint onto decisions.** The sweep becomes: three parallel passes at one commit, their findings combined, each carrying a reproduction that is verified, fixed in one bounded round, residue recorded. How they are combined is the one part of this option that changed at implementation; see the Decision. Edwin sees the decisions owed — not the findings — and nothing waits on him that he would approve every time.

## Decision

**Option 5**, accepted 2026-09-04.

**One amendment, provisional, and its first justification was wrong.** Option 5 as written kept "findings at least two of the three report"; the implementing session replaced it with *union the findings, then verify each against its reproduction*, arguing that passes 11 and 12 found 25 findings each and overlapped on almost nothing.

**That argument does not hold, and the independent review caught it.** Pass 11 ran at `19ba330` and pass 12 at `e2bee28`, after every one of pass 11's findings had been fixed. Two passes reading different corpora were guaranteed not to overlap, which says nothing about what three passes at *one* commit would do. That overlap rate has never been measured here.

What the data does support is narrower and still useful: **one pass misses real defects.** [[ISS-0051-The-Verification-Hook-Blocks-Every-Feature-That-Follows-The-Acceptance-Rule|ISS-0051]] existed at `19ba330` and pass 11 did not report it. That argues for running three passes; it is not an argument about how to combine them.

Union is therefore adopted **as a provisional default on a reason, not a measurement**: discarding a true finding costs more than verifying a false one, and verification is cheap. Majority voting works in the study that recommended it because every reviewer there read one artifact with known injected errors; whether it works here is criterion 4 below.
