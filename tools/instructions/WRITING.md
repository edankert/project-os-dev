...
11. **A note says what is true now.** When a task, feature or requirement changes, rewrite it to its current state rather than adding a dated section below the old text. The story of how it got there goes in the change note and the commit message. A standing record, such as an ADR, keeps at most a short dated decision log. Reason: every later reader reads the whole note to find the current state. In this repo TASK-0125 carried four dated sections of history, and your-trainer's FEAT-0122 about forty paragraphs of dated evidence (project-os-dev ISS-0098).
12. **Link a rule by name; do not restate it.** A note that depends on a project rule names the file and heading, such as `TESTING.md`, "The walk", rule 9. Reason: when the rule changes, a copy is wrong, and a search returns the copies alongside the rule (project-os-dev ISS-0092).
