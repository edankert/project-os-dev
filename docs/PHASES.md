...
| [[PHASE-0004-The-Walk\|PHASE-0004]] | The walk: a procedure over the owed checks | `done` | A generated walk sheet over the owed acceptance checks: the changed surfaces first, then sittings in an order authored once per project in WALK.md, each check's setup, steps and expected result inline. FEAT-0029, REQ-0028, ADR-0029, TASK-0110–TASK-0115. Rendered by project-os-cockpit (its FEAT-0149), consumed first by your-trainer (its FEAT-0119) |
| [[PHASE-0005-The-Walk-Reads-As-A-Script\|PHASE-0005]] | The walk reads as a script | `done` | The walk sheet opens with the app screens a release changed (a rider-facing sentence from each change note, before and after captures, no test ids), then one written procedure per sitting that `walk-sheet.py --check` holds to the owed set. Surfaces are screens by default. FEAT-0030, FEAT-0031, REQ-0029, ADR-0044 and ADR-0045 (both accepted), TASK-0116–TASK-0123. Rendered by project-os-cockpit (its PHASE-044, FEAT-0150), consumed first by your-trainer (its PHASE-024, FEAT-0120, FEAT-0121) |
| [[PHASE-0006-Codex-Native-Adapter\|PHASE-0006]] | Codex native project adapter | `active` | Generate native Codex skills and agents, install lifecycle hooks, test payload handling, and document trust and coverage. FEAT-0032 in review, REQ-0030 approved, TASK-0124 done, TST-0012 active |
| [[PHASE-0007-Reviews-That-Fix-And-A-Backlog-That-Is-True\|PHASE-0007]] | Reviews that fix, and a backlog that is true | `active` | A review covers one feature's diff, follows four fixed steps and stops at a budget. A finding in the feature's own code is fixed before the feature closes, and an issue names what a user would notice. Every open issue in your-trainer, project-os-cockpit and this repo is checked against the code once. FEAT-0034–FEAT-0036, ADR-0047 (proposed), ISS-0028, ISS-0062, ISS-0033–ISS-0039
