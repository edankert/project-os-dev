...
- Trigger: at git pre-push, and in CI.
- Rule: `QUALITY.md` — an executable test records no verdict; the run is the verdict (project-os-dev ADR-0025).
- Entrypoint: `python3 tools/scripts/run-tests.py` at pre-push; `run-tests.py --ci` in CI. Install with `bash tools/scripts/install-git-hooks.sh`.
- Check logic:
