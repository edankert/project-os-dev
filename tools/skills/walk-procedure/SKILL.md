...
6. **Under each step, write one expectation line per thing to observe.** Prefer the tags alone, `` - `TST-####.N` ``: the sheet and the cockpit print the check's current Expect words for it, so a later rewording of the check breaks nothing (`../../instructions/TESTING.md`, "The walk", rule 9; project-os-dev ADR-0049). A tag names Expect line N when the check has one Expect line per numbered step, and all its Expect lines otherwise; quote the line instead when you mean one of several unpaired lines. A quoted line ends with its tags too, and may carry several only when every check it names words that expectation identically — the validator compares the quote against each of them.
...
- **Do not paraphrase an `## Expect` line.** A tick recorded from a procedure step stands as a verdict on the check itself, and it only stands because the line says what the check says (project-os-cockpit ADR-0041). If the check's wording is wrong, fix the check's wording, in its note; a tag-only line then prints the fixed words, and `python3 tools/scripts/walk-tags.py --refresh` re-quotes a quoted one.
