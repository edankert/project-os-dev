...
## Definition of Done
- [x] The hint says that a delegation to the planner carries the user's prompt verbatim plus one sentence on what the result enables.
- [x] The planner prompt in `generate-adapters.py:63-78` says to expect both, and where the verbatim text lands in the note ([[TASK-0105]]'s "As reported" blockquote).
- [x] The generator is re-run and `.claude/agents/planner.md` committed with it.

## Steps
- [x] One sentence in each of the two strings.
...
Landed as template commit `f6ac538` on 2026-09-03. The hint carries the sentence (3e5c1b3); the planner prompt gained rule 6 and says to ask for a missing prompt or reason in its first line rather than classify a paraphrase.
