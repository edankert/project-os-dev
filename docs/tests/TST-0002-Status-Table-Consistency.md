...
artifacts: []
evidence: []
adequacy: "Verified by inversion across five rounds, 2026-07-26. Final suite: one branch reproducing each of ISS-0011/0012/0013/0014/0015 verbatim, plus the collection shapes and name forms that previously evaded (dict, set, frozenset, list, nested to depth 9, self-referential, underscore- and lowercase-named) and note-type renames in all three type tables. Separately: metrics parity shown across all 18 keys and 11 repos, and the STATUS-VALUE union fix measured against hash randomization (6/12 PYTHONHASHSEED values caught before, 12/12 after)."
...
| [[ISS-0015]] | the walk capped at depth 4 while promising any depth; `STATUS-VALUE` picked its type by hash order | yes |
...
At module scope the walk is shape-agnostic and name-agnostic: it recurses into tuple, list, set, frozenset and **dict**, to genuinely any nesting depth (unbounded, cycle-safe by identity — it capped at depth 4 until [[ISS-0015]], which is defensible code and an indefensible pair with a docstring promising "any depth"), under any name including underscore-prefixed ones, and exempts only `_NON_STATUS_COLLECTIONS` itself by identity. Getting there took four rounds — it checked only `tuple` and only `.isupper()` names until [[ISS-0013]], and still skipped `dict` and `_`-names until [[ISS-0014]], which mattered because `PHASE_RESOLVED`, the file's most-used table, is a dict.

The type tables are asserted separately: `COLLECTION_TYPE`, `TERMINAL_TYPES` and `METRIC_PREFIX_TYPE` hold note types rather than statuses, and a type renamed in one but not `ALLOWED_STATUS` fails just as silently. That assertion found a live bug on its first run — `decision`, an accepted alias `COLLECTION_TYPE` had always known, was missing from `ALLOWED_STATUS`, so decision-typed notes were checked against `adr` or `decision` by hash seed -- a per-run coin flip, not a dead gate (corrected in ISS-0015).
...
## Independent re-review (2026-07-26, model:claude-fable-5, round four, commit 5c4319d)

Authored by model:claude-opus-5 (commit trailer on 5c4319d), reviewed by model:claude-fable-5 — same model family, so this remains harm reduction, not the cross-vendor independence QUALITY.md asks for; a different-family or human pass is still owed.

**Every ISS-0014 fix is verified on the merits, including the headline one.** The file is structurally clean: 1619 lines, exactly one `if __name__` guard, zero duplicated top-level definitions by independent AST scan, and the full `12a7c70 → 5c4319d` diff (301 lines, reviewed in its entirety) contains only the intended ISS-0013 + ISS-0014 changes — no duplicated or dead code anywhere. All **12** fleet files are byte-identical (`cmp`; 11 repos — including `project-os-bench`, which the CHG's older 11-file table predates — plus the cockpit bundle), all 11 repos validate 0 errors, `--self-check` 0, `sync-snapshot --check` 0, cockpit suite 315 passed. The walker now catches everything ISS-0014 listed: dict-shaped, underscore-named, nested, lowercase, set/frozenset/list — all re-induced independently and each fails; nine targeted mutations (the ISS-0011 and ISS-0012 repros verbatim, `REVIEW_SETTLED_STATUSES` drift, a type renamed in each of `COLLECTION_TYPE`/`TERMINAL_TYPES`/`METRIC_PREFIX_TYPE`, the `decision` key deleted from `ALLOWED_STATUS`, and both prefix-map cases) fire with exactly the claimed messages — and deleting `REL` is silent exactly as the qualified row now states. "No multi-value inline status collection remains" was re-verified with an independent AST scan: exact. Metrics parity was confirmed by a different method than the author's (the 12a7c70 validator run against every repo's *current* recorded metrics: zero `METRICS` diffs in all 11 repos, so old == recorded == new on all 18 keys), and the `METRICS` gate still fires on an induced corruption. The `decision` alias is genuine: exactly one decision-typed note exists fleet-wide (your-health ADR-0006, `accepted` — two other grep hits are body-text mentions), the vocabulary matches STATUSES.md's `[[adr]]` line, and deleting the key turns `--self-check` red.

**Why changes-requested a fourth time — filed as [[ISS-0015]], the narrowest round yet.** (1) **"To any nesting depth" is false.** `_holds_strings` stops at `depth > 4`: a module-level status collection with a string at nesting depth 5 leaves `--self-check` green (demonstrated; depth 4 is caught). This note and the docstring say "any nesting depth"; ISS-0014's Resolution and the commit message correctly say "depth 4" — the descriptions disagree, and the sentence directly above this section says the boundary is "stated this precisely because the imprecise version has now been wrong three times". No real table nests past depth 3, so the code is adequate; the sentence is the defect, for the fifth time. (2) **"Validated against nothing" mischaracterises the decision-alias hole, and the true mechanism is a live residual.** STATUS-VALUE picks its type with `next(iter(expected_types), None)`, and for `decisions` that set has two members — demonstrated against 12a7c70 with a bogus decision status: caught on 5 of 10 `PYTHONHASHSEED` values, silent on the other 5. A per-run coin flip, not a dead gate. The alias fix makes it 10/10 only because both vocabularies currently coincide; a downstream repo customising `[[adr]]` in its STATUSES.md (no repo defines `[[decision]]`) silently re-splits them and the seed-dependence returns. The requested changes: state the depth cap exactly (or replace it with a cycle-safe unbounded walk), correct the "validated against nothing" sentence everywhere it appears, and make STATUS-VALUE check against the union of the expected types' vocabularies.
