...
**Option 3.** A `SUR-*` note names a screen unless its `kind` says otherwise. `TAXONOMY.md` gains these rules, stated there once:
...
- There will be more surfaces than the 12 to 15 project-os-cockpit FEAT-0130 aimed for. The top-level screens stay a set a person can hold in their head; children sit under them.
...
- [ ] **Where the gallery-key-to-surface mapping lives is decided**: on the surface note (a `captures:` field) or in a separate file the gallery tool owns. Say which, with the reason.
