...
## Definition of Done
- [ ] `issue-intake/SKILL.md` step 1 says to check in only when different readings lead to materially different work; otherwise implement the reading the wording most directly supports and record the assumption in the note.
- [ ] The check itself stays mandatory and still runs on every intake. What changes is the action on a term with two harmless readings, not whether the check happens.
- [ ] The planner's rule 5 in `tools/scripts/generate-adapters.py` reads: allocate and draft what is settled, and return the ambiguities as questions beside it.
- [ ] The generator is re-run and `.claude/agents/planner.md` committed with it.

## Steps
- [ ] Edit the five bullets of the check so the threshold applies to all of them, not only the first.
- [ ] Edit the planner prompt string; keep the sentence that ambiguity is upstream of documentation.
