...
## Outputs
- Regenerated native artifacts: `.claude/skills/`, `.claude/agents/`, `.agents/skills/`, `.codex/agents/`, `.cursor/rules/` (via the generator; never hand-edited).
...
## Checklist
1. **Regenerate derived artifacts:** run `python3 tools/scripts/generate-adapters.py --install-hooks` from the repo root. This rewrites Claude, Codex, and Cursor generated files from the canonical sources, installs/merges the Claude Code hook set, and installs Codex hooks only if `.codex/hooks.json` is absent. Existing Codex hooks require a manual merge or a reviewed `--force-hooks`. Verify with `--check`.
...
6. Preserve project-specific sections in downstream repositories when regenerating root files; generated `.claude/`/`.agents/skills/`/`.codex/agents/`/`.cursor/` artifacts carry a do-not-edit header and may be overwritten freely.
