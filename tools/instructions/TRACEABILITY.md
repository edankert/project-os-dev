...
- **A design past `draft` must have something to look at**: pictures in the note, or an HTML page in `asset:`. Pictures are the normal form (`../skills/design-authoring/SKILL.md`); `asset:` is optional. `DESIGN-ASSET` reports a design that shows nothing, and one whose declared `asset:` does not resolve; `DESIGN-ORPHAN` reports an HTML file no note claims, by `asset:` or by a link.
- Revisions are **commits against the design** — the note and its pictures, or its page — not new notes. A design that spawns a note per revision is being used as a changelog, and the reason for each revision belongs in the commit message where it survives the tool.
