...
alternatives:
  ...
  - "Keep requirement→feature many-to-many and gate the last feature to close — rejected: makes closure order-dependent (whoever closes last inherits a sibling's unfinished criteria); only 11 of 444 requirements are many-to-many, so deleting the case is cheaper than modelling it"
...
consequences:
  ...
  - "`implements:` names at most one feature (scalar, not a list); the 11 existing many-to-many requirements are migrated by picking the true owner or splitting + duplicating criteria — duplication is deliberate and may drift, accepted for 11 notes as the price of unambiguous ownership"
  ...
  - "The missing reverse gate is added: a feature cannot be `done` while any requirement that names it has unresolved criteria (unless that requirement is descoped) — applied FORWARD-ONLY; the 53 already-`done` features whose linked criteria are unresolved are grandfathered"
  ...
  - "Migration touches template + validator, ~79 `verified → implemented` demotions, 11 many-to-many splits/reassignments; the feature-less requirements are triaged separately (see follow-up; [[ISS-0005-Feature-Less-Requirement-Triage]] resolved 14 of 23 and left 9 for a filing decision)"
supersedes: ""
superseded: ""
...
`implements:` is a **scalar** (`[[FEAT-…]]`) or empty — never a list of two or more. A requirement naming two or more features is a validator error. The 11 existing many-to-many requirements are migrated case by case: pick the true owner and drop the rest, or split into per-feature requirements and duplicate the shared criteria. Duplication can drift and is accepted deliberately for 11 notes — unambiguous ownership is worth more than normalisation at this scale.
...
Same-family independent review (`model:claude-fable-5`) re-derived every quantitative claim in this ADR and found four wrong. They are corrected above; recorded here because a decision record whose central lesson is "ad-hoc scans under-report" should not quietly fix its own bad counts.

| Claim as written | Actual | Source of the error |
|---|---|---|
| 114 grandfathered features | **53** | ad-hoc scan, not the validator's own FEATURE-REQ output |
| ~200 unresolved criteria | **~900** | same |
| "375 of 385, only 10 many-to-many" | **410 of 421 linked, 11 many-to-many, 444 notes total** | arithmetic derived from the defective 59-orphan figure (385 = 444−59) |
| "7 of 9 repos have requirements" | **8 of 9** | project-os-dev's own 15 requirements were overlooked |

The 10→11 many-to-many slip was also mis-explained here as a bare-ID parsing miss; the real cause is the single-line/block-form defect described in [[ISS-0005-Feature-Less-Requirement-Triage]]. Corrected above.

Claims that survived re-derivation unchanged: 79 `verified` requirements with 56 (71%) referencing no test and **0** carrying a waiver; 190 `done` features with 177 (93%) carrying neither test nor waiver; the 23-not-59 orphan correction; the your-sudoku REQ-0086/0087 finding.

## Follow-up (out of scope for this ADR)

The 23 feature-less requirements are triaged separately, because they are not one kind of thing:
...
5. Data migration across repos — demote 79 `verified`, split/reassign 11 many-to-many. The remaining feature-less requirements are handled by the separate triage above.
...
- **11 REQ-OWNER violations** resolved — one more than the 10 first forecast, because the scan behind that estimate read only single-line `implements:` and missed block-form YAML lists (see [[ISS-0005-Feature-Less-Requirement-Triage]] for the same defect at larger scale). Each kept its true owner in `implements:`; the displaced features moved to `related:` so no link was lost.
...
- **Forward-only worked as designed**: **53** grandfathered features carrying **~900** unresolved criteria between them report as warnings, not errors, so no repo's CI broke. (An earlier draft of this section said 114 features and ~200 criteria; both came from an ad-hoc scan rather than the validator, and independent review caught them — see "Numbers corrected" below.) Post-cutover work is held to the gate.
- The parity test written for the earlier Delivered band ([[TST-0019]] in project-os-cockpit) failed 6 of 13 cases the moment `statuses.py` changed, naming the stale surfaces — the intended behaviour, and the reason the cockpit half took one pass instead of six.
