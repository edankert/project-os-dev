...
- [x] **`RULE-ONCE` reconsidered, 2026-09-04, and the answer is narrower than one check** — evidence: [[ADR-0026-When-A-Drift-Sweep-Stops|ADR-0026]], accepted by user:edwin. The condition written above ("reconsider when the corpus reaches zero") never arrived and, on twelve passes, cannot: the counts ran 36, 26, 2, 3, 2, 2, 0, 21, 2, 5, 25, 25. So the reconsideration happened on the opposite evidence — not a corpus at zero, but one holding steady at ~25 findings per pass. A single `RULE-ONCE` check is still declined, for the reason given above: it needs a registry of rule phrases that does not exist. What replaces it is four narrow checks over the classes the sweep kept rediscovering, listed in ADR-0026's Acceptance. The first, `BASE-STATUS`, is built and errors from day one on a measured count of zero; the other three are [[ISS-0052-Three-More-Drift-Classes-Should-Be-Checks|ISS-0052]].
