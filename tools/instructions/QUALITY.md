...
- Do not mark a feature `done` while any requirement that names it in `implements:` still has an unresolved acceptance criterion (validator FEATURE-REQ). Satisfy the criteria, reconcile them, or descope the requirement (`deferred`/`cancelled`/`superseded`) first. A feature closing over its own unbuilt requirements is the failure this gate exists to stop; it is enforced **forward-only**, so features that closed before ADR-0007 landed are not retroactively invalidated.
  - Mechanically, "forward-only" is keyed on the note's `updated:` date against the validator's `FEATURE_REQ_GATE_FROM` cutover (2026-07-25), **not** on when the item actually closed — the validator cannot see close dates. Two consequences worth knowing: editing a grandfathered note for any reason re-arms the gate on it (bring it into compliance, or leave the note alone), and an item closed after the cutover whose `updated:` is stale or malformed is treated as grandfathered — the gate fails open rather than guessing.
