...
- Inversion: a fixture repo where each of the six conditions is violated in turn, asserting the entry survives in every case. **Condition 3 needs a deliberately illegal fixture**: `deferred` and `done` are mutually exclusive (`STATUSES.md`), so a well-formed corpus cannot violate condition 3 without also violating condition 1 — an implementation that omits the deferred check entirely would pass a naive inversion suite. Assert it against a hand-built entry that is both, or assert the check exists by other means.
