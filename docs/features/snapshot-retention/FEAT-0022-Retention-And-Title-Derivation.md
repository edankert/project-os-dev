...
- **`title` joins the derived set**, so the 579 drifted titles across the fleet are reconciled once and cannot recur.
...
- **This is the first operation in the system that deletes lines from a tracked file**, and it will run in twelve repos. `TASK-0072`'s fleet-rollout discipline applies: dry-run everywhere, inspect diffs, per-repo commits.
