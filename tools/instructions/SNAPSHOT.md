...
- `ci` (object, optional): `ci.suite_command` is the one command CI runs instead of every `TST-*` `command:` (`HOOKS.md` HC-009). Declare it where the test notes carry filtered commands that the suite already covers, e.g. `suite_command: "./gradlew test --rerun-tasks"`. Absent means CI runs every command.
